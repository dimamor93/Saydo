...
class LocalLLMProvider(LLMProvider):
    def __init__(self, model="qwen3.8:27b", base_url="http://127.0.0.1:11434", timeout=60.0):
        self.model=model; self.base_url=base_url.rstrip("/"); self.timeout=timeout; self._styles=StyleStore()
    def _system_prompt(self):
        prompt=str(self._styles.get_selected().get("prompt","")).strip()
        if not prompt: return SYSTEM_PROMPT
    ...
    def process(self,text):
        payload={"model":self.model,"system":self._system_prompt(),"prompt":text,"stream":False,"think":False}
        request=urllib.request.Request(f"{self.base_url}/api/generate",data=json.dumps(payload).encode("utf-8"),headers={"Content-Type":"application/json"},method="POST")
        try:
            with urllib.request.urlopen(request,timeout=self.timeout) as response: data=json.loads(response.read().decode("utf-8"))
        except urllib.error.URLError as exc: raise RuntimeError(f"Could not connect to Ollama: {exc}") from exc
        result=data.get("response","").strip()
        if not result: raise RuntimeError("Ollama returned an empty response.")
        return result
